...
export default function LoginScreen() {
  ...
  const handleVerifyOtp = useCallback(async (code: string) => {
    ...
    // If it's a new user, Supabase sends a signup OTP which requires type: 'signup'
    if (res.error && res.error.message.toLowerCase().includes('token')) {
      res = await supabase.auth.verifyOtp({
        email: normalizeEmail(email),
        token: code,
        type: 'signup',
      })
      // If signup fails, try magiclink as a last resort for older Supabase configurations
      if (res.error && res.error.message.toLowerCase().includes('token')) {
        res = await supabase.auth.verifyOtp({
          email: normalizeEmail(email),
          token: code,
